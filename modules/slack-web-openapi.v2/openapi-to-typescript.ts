...
export function to(name: string, schema: JSONSchema, getReferenced: GetReferenced, addDependency: AddDependency): t.TypeReference {
    ...
    if (!schema.type && schema.hasOwnProperty("items")) {
        return t.unionCombinator((schema as any).items.map((item: JSONSchema) => to(
            `${name}_item`,
            item,
            getReferenced,
            addDependency
    ...
    switch (schema.type) {
        ...
        case "array":
            return toArrayCombinator(name, schema as ArraySchema, getReferenced, addDependency);
        case "object":
            return toTypeCombinator(name, schema as ObjectSchema, getReferenced, addDependency);
    }
}
...
export function generate(directory: string, schema: OpenAPISchema) {
    ...
    function getFile(fileName: string, fileNamespace: string, typeName: string, file: string): string {
        ...
        return imports.concat([
            ...
            `export type Slack${typeName} = t.TypeOf<typeof ${typeName}>;`
    ]).join("\n");
  }
